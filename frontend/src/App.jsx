      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-8 pb-8">
        <div className="space-y-4">
          {messages.map((msg, idx) => (
            <div key={`${msg.role}-${idx}`} className={classNames("rounded-2xl p-4 text-sm leading-relaxed shadow-lg shadow-black/5", msg.role === "system" ? "bg-white/10 text-emerald-100" : "bg-white/5")}>
              <p className="mb-2 text-[11px] uppercase tracking-[0.3em] text-white/60">{msg.role === "system" ? "System" : "Advisor"}</p>
              <p>{msg.text}</p>
            </div>
          ))}
        </div>
      </div>
      <div className="relative z-10 border-t border-white/10 px-8 py-4 text-xs text-white/50">Session notes stay visible across every step.</div>
    </aside>
  );
}
...
function UploadStep({ selectedFile, fileInputRef, onFileChange, onUpload }) {
  return (
...
export default function App() {
  ...
  const handleDatesNext = async () => {
    ...
    try {
      const res = await fetch("/api/trail/match", {
        ...
        body: JSON.stringify({ route, name_hint: "" }),
      });
      const data = await res.json();
      setTrailMatch(data);
      if (data.auto_selected) {
        setSelectedTrailId(data.auto_selected.id);
        addMessage("assistant", `Best match: ${data.auto_selected.name} in ${data.auto_selected.area}`);
      } else if (data.shortlist?.length) {
        addMessage("assistant", `Found ${data.shortlist.length} candidate trails.`);
      } else {
        addMessage("assistant", "No matching trails found.");
      }
    } catch (err) {
      addMessage("assistant", "Could not match trail. Please try another GPX file.");
    }
    setLoading(false);
  };
  ...
  const renderStep = () => {
    switch (step.id) {
      case "upload":
        return <UploadStep selectedFile={selectedFile} fileInputRef={fileInputRef} onFileChange={handleFileChange} onUpload={handleUpload} />;
      case "dates":
  ...
  return (
    <div className="grid min-h-screen grid-cols-[1.05fr_2fr]">
